...
function taskLabel(t: TaskEntry): string {
  if (t.label?.trim()) return t.label;
  const parts = [t.connection, t.funcName, t.input].filter(Boolean);
...
function resolveConnectionForTask(task: TaskEntry, connections: Connection[]): Connection | null {
  ...
  return connections.find((connection) => connection.name.trim().toLowerCase() === key) ?? null;
}
...
export default function ProjectDashboard() {
  ...
  const [autoSaveEnabled] = useState(() => readDashboardAutoSaveDefault());
  ...
  const runDashboardButton = useCallback(async (item: CanvasItem) => {
    ...
    try {
      ...
      if (hasSuccessfulExecutionResult(response.body?.results)) {
  ...
  const canvasRClick = (e: React.MouseEvent) => {
    e.preventDefault();
    setCtxMenu({ sx: e.clientX, sy: e.clientY });
  };
          ...
          if (item.type === "button") {
            ...
            return (
              <button
                ...
                title={
                  hasConnectionIssue
                    ? "One or more task devices are offline, disabled, or missing."
                    : (hasRunnableTasks
                        ? `${isTallied ? "Tallied - " : ""}Run ${enabledTaskCount} task(s)`
                        : (item.tasks.length > 0 ? "All tasks are excluded" : "No tasks configured"))
                }
              >
                {hasConnectionIssue ? (
                  <>
            ...
            <div ref={canvasRef}
              ...
              {ctxMenu && (
                <CtxMenu
                  ...
                  onAddButton={() => { const {x,y}=canvasXY(ctxMenu.sx,ctxMenu.sy); finishCreate("button",x,y,136,50); }}
                  onAddLabel={() => { const {x,y}=canvasXY(ctxMenu.sx,ctxMenu.sy); finishCreate("label",x,y,120,28); }}
                  onDuplicateSelected={() => {
                    if (!selId) return;
                    duplicateCanvasItemById(selId);
                    setCtxMenu(null);
              ...
              {showTaskWorkspace && editorItem && (
                <motion.div
                  ...
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ duration: 0.14 * animationDurationScale, ease: [0.22, 1, 0.36, 1] }}
                ...
                >
                  <motion.div
                    ...
                    transition={{ duration: 0.18 * animationDurationScale, ease: [0.22, 1, 0.36, 1] }}
                  >
                    <div
                                ...
                                {workspaceTasks.map((task) => {
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      ...
                                      >
                                        {taskLabel(task)}{!isEnabled ? " [Excluded]" : ""}
                                      </div>
                                      <button
                                    ...
                                    {editorItem.tasks.length === 0 ? (
                                      ...
                                      editorItem.tasks.map((task, i, arr) => {
                                        const lbl = task.label?.trim()
                                          || [task.connection, task.funcName, task.input].filter(Boolean).join(" - ")
                                          || "(empty)";
                                        return (
                                          <div
                                            key={task.id}
    ...
    >
      ...
      {hasSelection && (
        <>
          ...
          <CtxBtn label="Delete" onClick={onDeleteSelected} color="#D1D5DC" />
