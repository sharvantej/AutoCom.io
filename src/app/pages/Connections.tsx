...
function buildConnectionPayloadFromForm(form: FormState, selectedProjectId: number | null, existingId?: number): Connection {
  ...
  return {
    ...
    ...atemFields,
    active: false, // Always inactive on creation/update, let user toggle
...
export default function Connections() {
  const { connections, setConnections, projects } = useAppContext();
  const t = useTheme();
  ...
  const filteredConnections = selectedProjectId === null
    ? connections
  ...
  const handleSaveConnection = async () => {
    ...
    try {
      const updatedPayload = buildConnectionPayloadFromForm(form, selectedProjectId, editingConnection.id);
        ...
        <div className="px-[16px]" style={{ paddingTop: 16 }}>
          <div className="flex items-center justify-between" style={{ height: 35 }}>
            ...
            <div className="relative" ref={projectDropdownRef}>
              <motion.button
                className="text-[14px] px-[12px] flex items-center gap-[8px] hover:bg-[var(--btn-hover)] transition-colors border border-transparent"
                style={{ height: 35, minWidth: 164, backgroundColor: t.btnSecondary, color: t.textPrimary }}
                onClick={() => setProjectDropdownOpen(v => !v)}
                whileTap={{ scale: 0.97 }}
                transition={{ duration: 0.1 }}
              >
                <span className="flex-1 text-left">{selectedProjectName}</span>
                <motion.span
              ...
              <AnimatePresence>
                {projectDropdownOpen && (
                  <motion.div
                    className="absolute right-0 top-full z-50 border card-pop"
                    style={{
                      minWidth: 164,
                  ...
                  >
                    {[{ id: null as number | null, name: "All Projects" }, ...projects].map(p => (
                      <button
                        key={p.id ?? "all"}
                        className="w-full text-left px-[11px] py-[8px] text-[12px] hover:bg-[var(--ctx-hover)] transition-colors"
                        style={{
                          color: t.textPrimary,
                          backgroundColor: selectedProjectId === p.id ? t.navActive : undefined,
                        }}
                        onClick={() => {
