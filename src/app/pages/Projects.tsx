...
function filterProjectConnections(
  connections: Connection[],
  projectId: number,
) {
  return connections
...
function parseImportBundle(raw: unknown, sourceFileName?: string): ProjectImportBundle | null {
  ...
  return {
    projectName,
    connections: normalizeImportedConnections(connectionsRaw, [], 0),
    layout,
  };
}
...
export default function Projects() {
  ...
  const handleDelete = (id: number)  => {
    ...
    setConnections(prev => prev.map((connection) => (
      connection.projectId === id ? { ...connection, projectId: null } : connection
    )));
    setContextMenu(null);
  };
  ...
  const handleExport = async (project: Project) => {
    ...
    try {
      ...
      const projectConnections = filterProjectConnections(connections, project.id);
  ...
  const handleImportFileSelection = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setConnections((prev) => [
        ...prev,
        ...normalizeImportedConnections(imported.connections, prev, nextProjectId),
      ]);
      if (isTauri()) {
    ...
    <>
      <div className="flex-1 overflow-y-auto page-pop" style={{ backgroundColor: t.bgContent }}>
        <div className="px-[16px]" style={{ paddingTop: 16 }}>
          ...
          <div className="flex items-center justify-between gap-[8px]">
            <div className="flex items-center gap-[8px]">
              <button
                ...
                style={{ height: 35, backgroundColor: "#101828", color: t.textPrimary }}
          ...
          {/* Project rows */}
          <div style={{ marginTop: 16, backgroundColor: "#101828" }}>
            {projects.map(project => (
              <div
                key={project.id}
                className="flex items-center justify-between py-[10px] px-[16px] group card-pop
              ...
              >
                ...
                <div className="flex items-center gap-[8px]">
                  <button
                    className="h-[26px] w-[26px] opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center border"
                    style={{ color: t.textMuted, borderColor: t.inputBorder, backgroundColor: "#101828" }}
                  ...
                  <button
                    className="h-[26px] px-[8px] border flex items-center gap-[6px] text-[13px] transition-colors"
                    style={{ color: t.textPrimary, borderColor: t.inputBorder, backgroundColor: "#101828" }}
      ...
      {modal === "new" && (
        <ModalOverlay onClose={() => setModal(null)}>
          <ModalBox title="New Project" onClose={() => setModal(null)} t={t} widthClass="w-[420px]">
            <div className="flex flex-col" style={{ gap: 16 }}>
              <div style={{ fontSize: 12, color: t.textMuted }}>
                Add a new project
              </div>
              <div className="flex flex-col" style={{ gap: 7 }}>
                <span style={{ fontSize: 13, color: t.textPrimary }}>Project Name</span>
                <input
                  autoFocus
              ...
              <div className="flex" style={{ gap: 12, paddingTop: 6 }}>
                <button
                  className="flex-1 border transition-opacity hover:opacity-90"
                  style={{
                    height: 40,
                    backgroundColor: t.inputBg,
                    borderColor: t.ctxBorder,
                    color: t.textPrimary,
                    fontSize: 13,
                  }}
                ...
                <button
                  className="flex-1 flex items-center justify-center gap-2 border transition-opacity hover:opacity-90"
                  style={{
                    height: 40,
  ...
  return (
    <div className={`${widthClass} p-6 shadow-2xl border card-pop`}
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button className="transition-colors" style={{ color: t.textMuted }}
          onMouseEnter={e => (e.currentTarget.style.color = t.textPrimary)}
          onMouseLeave={e => (e.currentTarget.style.color = t.textMuted)}
          onClick={onClose}>
          <X size={15} />
...
function ModalActions({ onCancel, onConfirm, confirmLabel, confirmIcon, t }: {
  ...
  return (
    <div className="flex justify-end gap-2 mt-5">
      <button
        className="px-4 py-[7px] text-[13px] border hover:bg-[var(--btn-hover)] transition-colors"
        style={{ color: t.textPrimary, borderColor: t.ctxBorder }}
        onClick={onCancel}
      >
        Cancel
      </button>
      <button
        className="flex items-center gap-2 px-4 py-[7px] text-[13px] border hover:bg-[var(--btn-hover)] transition-colors"
        style={{ color: t.textPrimary, backgroundColor: t.navActive, borderColor: t.ctxBorder }}
        onClick={onConfirm}
      >
