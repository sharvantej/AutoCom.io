...
import { buildVmixCommand } from "./vmixShortcuts";
...
function isVmixConnection(connection: Connection): boolean {
  return String(connection.device ?? "").trim().toLowerCase() === "vmix";
}
...
function buildShortcutRow(task: TaskEntry, connection: Connection): Record<string, unknown> {
  ...
  const vmixConnection = isVmixConnection(connection);
  ...
  // Surface common fields for transports that expect top-level command/lineEnd.
  if (typeof params.command === "string") {
    const rawCommand = params.command.trimEnd();
    const command = vmixConnection &&
      rawCommand &&
      !/^FUNCTION\s+/i.test(rawCommand)
      ? `FUNCTION ${rawCommand}`
      : rawCommand;
    row.command = command;
  ...
  if (protocol === "tcp") {
    const lineEnd = typeof params.lineEnd === "string"
      ? params.lineEnd
      : (vmixConnection ? "crlf" : "none");
    row.lineEnd = lineEnd;
    params.lineEnd = lineEnd;
  } else if (typeof params.lineEnd === "string") {
    row.lineEnd = params.lineEnd;
  }
...
function buildGenericManualRow(task: TaskEntry, connection: Connection): Record<string, unknown> {
  ...
  if (isVmixConnection(connection)) {
    ...
    let command = "";
    const hasFunctionLabel = !isPlaceholderFunctionLabel(funcName);
    if (hasFunctionLabel) {
      const vmixArgs: Record<string, string> = {};
      if (input) vmixArgs.Input = input;
      if (value) vmixArgs.Value = value;
      command = buildVmixCommand(funcName, vmixArgs);
    } else {
      command = (input || value || "").trim();
    }

    if (!command) {
      throw new Error(`vMix task "${task.id}" needs a function or command payload.`);
    }
    if (!/^FUNCTION\s+/i.test(command)) {
  ...
  if (protocol === "tcp" && typeof row.command === "string" && !row.command.endsWith("\r\n")) {
