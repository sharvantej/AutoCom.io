...
fn exec_task<'a>(
  ...
  Box::pin(async move {
    ...
    let payload = s(task.get("command").or_else(|| task.get("payload")).or_else(|| task.get("message")));
    if payload.is_empty() {
    ...
    let tcp_response = if save_tcp_response {
      send_tcp_capture(&host, port, &payload, &line_end(task.get("lineEnd"), "\r\n"), 250).await?
    } else {
      send_tcp(&host, port, &payload, &line_end(task.get("lineEnd"), "\r\n")).await?;
      None
    };
    emit_log(
      &task,
      &conn,
      format!("TCP {}", payload.chars().take(80).collect::<String>()),
      events,
    );
